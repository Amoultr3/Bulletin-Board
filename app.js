...
const state = {
  page:"dashboard", view:"week", cursor:new Date(), selectedDate:fmtDate(new Date()), selectedRows:new Set(),
...
function pin(item,occurrenceDate){
  ...
  button.addEventListener("dragstart",event=>event.dataTransfer.setData("text/plain",item.id));button.addEventListener("click",event=>{event.stopPropagation();if(state.view==="month")openDay(occurrenceDate);else openItemForm(item,occurrenceDate)});return button;
}
...
function renderItemsTable(){
  ...
  $$("#itemsTableBody tr").forEach(row=>{const item=state.items.find(x=>x.id===row.dataset.row);row.querySelector(".row-select").addEventListener("change",event=>{event.target.checked?state.selectedRows.add(item.id):state.selectedRows.delete(item.id);renderItemsTable();});row.querySelectorAll("[data-field]").forEach(control=>{const save=()=>{const field=control.dataset.field;if(field==="important")item.important=!item.important;else item[field]=control.value;if(field==="status")item.completed=control.value==="completed";saveAndSync();render();};control.addEventListener(control.tagName==="BUTTON"?"click":"change",save);});row.querySelectorAll("[data-edit-row],[data-open-notes]").forEach(button=>button.addEventListener("click",()=>openItemForm(item,item.date)));});
}
function deleteSelected(){if(!state.selectedRows.size)return;state.items=state.items.filter(item=>!state.selectedRows.has(item.id));state.selectedRows.clear();saveAndSync();render();toast("Selected items deleted");}
function importRows(){
...
function bind(){
  ...
  ["#sheetSearch","#sheetCategory","#sheetStatus"].forEach(selector=>$(selector).addEventListener("input",renderItemsTable));$("#selectAllRows").addEventListener("change",event=>{filteredSheetItems().forEach(item=>event.target.checked?state.selectedRows.add(item.id):state.selectedRows.delete(item.id));renderItemsTable();});$("#deleteSelectedButton").addEventListener("click",deleteSelected);$("#addRowButton").addEventListener("click",()=>openItemForm(null,fmtDate(new Date())));$("#importRowsButton").addEventListener("click",()=>$("#importDialog").showModal());$("#processImportButton").addEventListener("click",importRows);
...
bind();render();initSupabase();if("serviceWorker" in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./sw.js?v=4"));
